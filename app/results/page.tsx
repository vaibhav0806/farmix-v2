...
function ResultsComponent() {
  ...
  useEffect(() => {
    ...
    if (similarityScore) {
      ...
      setCommonFollowers(
        JSON.parse(decodeURIComponent(commonFollowers || "[]"))
      );
    ...
    <main className="flex flex-col items-center min-h-screen p-4 gap-4 bg-black text-white font-sans">
      ...
      <div className="w-full flex flex-col md:flex-row justify-around space-x-0 md:space-x-6 border border-gray-700 rounded-md p-4 bg-gray-900 bg-opacity-50 shadow-lg">
        <div className="w-full md:w-1/3 mb-4 md:mb-0">
          <h2 className="text-2xl mb-4 font-bold text-center border-b border-gray-700 pb-2">
            Common NFTs
          </h2>
                ...
                <div key={index} className="mb-4 p-2 border-b border-gray-600">
                  ...
                  {nft ? (
                    <img
                      src={nft}
                      alt={`NFT ${index + 1}`}
                      className="w-full h-24 object-contain mb-2 rounded-md shadow-md"
                    />
                  ) : (
          ...
          <div className="h-[21rem] scroll-smooth scrollbar bg-gray-800 bg-opacity-30 p-4 rounded-lg">
            {commonTokens.length > 0 ? (
              commonTokens.map((token, index) => (
                <div
                  key={index}
                  className="mb-2 flex flex-row gap-2 items-center border p-4 rounded-md bg-gray-700 hover:bg-gray-600 transition-colors shadow-md"
                >
      ...
      <Button
        variant="flat"
        color="secondary"
        className="lg:absolute lg:top-24 lg:right-8 text-md"
        onClick={() => {
          window.open(
            `https://warpcast.com/~/compose?text=Check%20out%20the%20similarity%20analysis%20between%20@${primaryUsername}%20and%20$@{secondaryUsername}.%20Discover%20shared%20NFTs,%20tokens,%20and%20followers%20and%20see%20the%20similarity%20score%20of%20${similarityScore.toFixed(
              2
            )}%25.%20View%20the%20analysis%20here:%20${window.location.href}`,
            "_blank"
          );
        }}
      >
